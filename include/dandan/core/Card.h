...
namespace dandan::core
{
    ...
    public:
        ...
        Card() = default;
        Card(std::string_view name, int cost, Type type, std::vector<std::unique_ptr<abilities::IAbility>> abilities = {})
            : m_name{name}, m_cost{cost}, m_type{type}, m_abilities{std::move(abilities)} {}

        std::string_view get_name() const { return m_name; }
        int get_cost() const { return m_cost; }
        Type get_type() const { return m_type; }
        const std::vector<std::unique_ptr<abilities::IAbility>> &get_abilities() const { return m_abilities; }
