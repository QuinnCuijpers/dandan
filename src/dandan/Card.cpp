...
void from_json(const nlohmann::json &j, Card &c)
{
    ...
    for (const auto &ability_json : j.at("effects"))
    {
        const std::string type = ability_json.at("type").get<std::string>();
        std::unique_ptr<IAbility> ability;
        if (type == "ManaAbility")
        {
            ability = std::make_unique<ManaAbility>();
        }
        else if (type == "WithDamage")
        {
            ability = std::make_unique<WithDamage>(nullptr, 0); // Placeholder, will be set in from_json
        }
        else
        {
