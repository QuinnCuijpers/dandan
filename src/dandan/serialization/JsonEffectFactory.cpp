...
nlohmann::json
JsonFactory<effects::IEffect>::create_json(const effects::IEffect *effect) {
  ...
  {
    throw std::runtime_error("Unknown effect type for JSON serialization: " +
                             std::string(typeid(*effect).name()));
  }
}

std::unique_ptr<effects::IEffect>
JsonFactory<effects::IEffect>::create_product(const nlohmann::json &j) {
