...
{
    nlohmann::json JsonFactory<events::IEvent>::create_json(const events::IEvent *event)
    {
        if (const auto *entersBattlefieldEvent = dynamic_cast<const events::EntersBattlefieldEvent *>(event))
        {
            auto j = nlohmann::json{{"type", "EntersBattlefieldEvent"}, {"data", nlohmann::json()}};
    ...
    std::unique_ptr<events::IEvent> JsonFactory<events::IEvent>::create_product(const nlohmann::json &j)
    {
