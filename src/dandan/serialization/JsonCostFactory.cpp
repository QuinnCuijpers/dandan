...
namespace dandan::serialization
{
    nlohmann::json JsonFactory<costs::ICost>::create_json(const costs::ICost *cost)
    {
        ...
        else if (const auto *generic_cost = dynamic_cast<const costs::GenericManaCost *>(cost))
        {
            auto j =
                nlohmann::json{
                    {"type", "GenericManaCost"},
                    {"data", nlohmann::json::object()}};
    ...
    std::unique_ptr<costs::ICost> JsonFactory<costs::ICost>::create_product(const nlohmann::json &j)
    {
